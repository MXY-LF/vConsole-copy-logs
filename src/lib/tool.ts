...
const _safeJSONStringify = (
  obj,
  opt: ISafeJSONStringifyOption,
  _curDepth = 0
) => {
...
export function isWxEnv() {
  return (
    ...
    !!(<any>window).__wxConfig &&
    !!(<any>window).wx &&
    !!(<any>window).__virtualDOM__
  );
}
...
export function removeLargeFields(obj, maxSize = 1000, depth = 0) {
  if (typeof obj !== "object" || obj === null) return obj;

  // if (depth > 2) {
  // 第五层及之后
  if (typeof obj === "string" && obj.length > maxSize) {
    return ""; // 清除大字符串
  } else if (Array.isArray(obj) && obj.length > maxSize) {
    // 假设每个元素平均占用10字节
    return []; // 清除大数组
  } else if (typeof obj === "object" && JSON.stringify(obj).length > maxSize) {
    return {}; // 清除大对象
  }
  return obj; // 返回第五层的普通对象
  // }

  if (Array.isArray(obj)) {
    return obj
      .map((item) => removeLargeFields(item, maxSize, depth + 1))
  ...
  for (const key in obj) {
    if (obj.hasOwnProperty(key)) {
      const value = obj[key];
      const cleanedValue = removeLargeFields(value, maxSize, depth + 1);
